...
func GetPackageIdFromRoot(ResourceData *schema.ResourceData,packageName string, username string, password string) (string, error) {
	...
	url.WriteString("current/")
	url.WriteString("package")
	log.Println("Not taking the project parameter in the configuration URL")
	log.Println("URL:>", url.String())
...
func CheckOut(ResourceData *schema.ResourceData, projectId string, username string, password string)([]byte, error){
	...
	if asset=="script"{
		...
		log.Println("<=====Hi, I went inside this loop=====>")
		assetid,err:=GetPackageIdFromRoot(ResourceData,assetname,username,password)
		if err != nil {
			log.Println("no package found with the name: ", assetname)
		}
		aid=assetid
